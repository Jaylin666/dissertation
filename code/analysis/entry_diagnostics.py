...
def compact_entry_cohorts(classification: pd.DataFrame) -> pd.DataFrame:
    labels = {
        "system_start_left_censored": "model-start / left-censored",
        "within_5y_burn_in_recorded_entry": "within 1986-1989 burn-in",
        "post_burn_in_recorded_entry": "1990-2024 post-burn-in recorded entrants",
        "test_year_recorded_entry": "2025 held-out recorded entrants",
    }
    rows = []
    for cohort, interpretation in labels.items():
        rows.append(
            {
                "cohort": cohort,
                "interpretation": interpretation,
                "players": int(classification["entry_cohort"].eq(cohort).sum()),
            }
        )
    return pd.DataFrame(rows)


def compact_prematch_scale(diagnostics: pd.DataFrame) -> pd.DataFrame:
    test = diagnostics.loc[diagnostics["first_recorded_year"].eq(TEST_YEAR)]
    values = [
        ("fixed_entry_anchor", INITIAL_RATING),
        ("mean_contemporaneous_established_rating", test["contemporaneous_established_mean_rating"].mean()),
        ("median_contemporaneous_established_rating", test["contemporaneous_established_median_rating"].median()),
        ("mean_initial_minus_contemporaneous_median", test["initial_minus_contemporaneous_median"].mean()),
        ("mean_active_established_365d_median", test["active_established_365d_median_rating"].mean()),
        ("mean_initial_minus_active_established_365d_median", test["initial_minus_active_established_365d_median"].mean()),
        ("mean_actual_first_opponent_rating", test["opponent_rating_before"].mean()),
        ("median_actual_first_opponent_rating", test["opponent_rating_before"].median()),
        ("mean_initial_minus_actual_opponent_rating", test["initial_minus_opponent_rating"].mean()),
        ("mean_current_first_appearance_probability", test["p_focal_current_convention"].mean()),
        ("empirical_first_appearance_win_rate", test["outcome_focal"].mean()),
        ("current_prediction_bias", test["p_focal_current_convention"].mean() - test["outcome_focal"].mean()),
        ("current_brier", test["brier_current"].mean()),
    ]
    return pd.DataFrame(values, columns=["metric", "value"])


def compact_orientation(cohort_summary: pd.DataFrame) -> pd.DataFrame:
    test = cohort_summary.loc[
        cohort_summary["group"].eq("test_year_recorded_entry")
    ].iloc[0]
    return pd.DataFrame(
        [
            {
                "group": "test_year_recorded_entry",
                "first_appearances": int(test["n_first_appearances"]),
                "mean_current_probability": float(test["mean_current_probability"]),
                "mean_direct_focal_probability": float(test["mean_direct_probability"]),
                "empirical_win_rate": float(test["empirical_win_rate"]),
                "current_prediction_bias": float(test["current_prediction_bias"]),
                "direct_prediction_bias": float(test["direct_prediction_bias"]),
                "current_brier": float(test["current_brier"]),
                "direct_brier": float(test["direct_brier"]),
                "qualitative_overprediction_direction_agrees": bool(
                    test["current_prediction_bias"] > 0 and test["direct_prediction_bias"] > 0
                ),
            }
        ]
    )
...
def create_prematch_scale_alignment_replot(core: pd.DataFrame, path: Path) -> Path:
    """Create Figure 5.3 from the compact prematch scale table."""

    values = core.set_index("metric")["value"]
    required = [
        "fixed_entry_anchor",
        "mean_contemporaneous_established_rating",
        "median_contemporaneous_established_rating",
        "mean_active_established_365d_median",
        "mean_actual_first_opponent_rating",
    ]
    if not set(required).issubset(values.index):
        raise ValueError("Prematch scale figure is missing a required metric")
    labels = [
        "Fixed entry\nanchor",
        "Mean established\nscale",
        "Median established\nscale",
    ...
    ax.bar(labels, [values[name] for name in required], color=["#C44E52", "#4C78A8", "#72B7B2", "#54A24B", "#F58518"])
    ax.set_ylabel("Prematch Glicko rating")
...
def _comparison_debut_probabilities(scores: pd.DataFrame) -> pd.DataFrame:
    rows: list[pd.DataFrame] = []
    for side in ("a", "b"):
        mask = robust_bool_series(scores[f"{side}_is_debut"], f"{side}_is_debut")
        player = f"player_{side}_id"
        probability = scores["p_a_Glicko_low_fixed"] if side == "a" else 1.0 - scores["p_a_Glicko_low_fixed"]
        rows.append(
            pd.DataFrame(
                {
                    "match_id": scores.loc[mask, "match_id"].astype(int),
                    "player_id": scores.loc[mask, player].astype(int),
                    "comparison_probability": probability.loc[mask].astype(float),
                }
            )
        )
...
def run_pipeline(
    ...
    """Run recorded-entry, burn-in, scale, and orientation diagnostics."""

    step24 = load_step24_module()
    matches, inactivity_unit, _ = step24.load_matches(matches_path)
    matches = matches.reset_index(drop=True)
    matches["match_sequence"] = np.arange(1, len(matches) + 1)
    low = next(
        variant
        for variant in step24.build_variants(inactivity_unit)
        if variant["variant"] == "low_inflation"
    )
    appearance_long = build_strict_appearance_long(matches)
    classification = build_strict_player_classification(appearance_long)
    diagnostics = run_frozen_low_inflation_state(matches, classification, step24, low)
    cohort_summary = build_entry_cohort_summary(diagnostics)
    burn_in = build_burn_in_sensitivity(diagnostics)
    entry_core = compact_entry_cohorts(classification)
    scale_core = compact_prematch_scale(diagnostics)
    orientation_core = compact_orientation(cohort_summary)

    expected_counts = [314, 456, 4297, 76]
    if entry_core["players"].tolist() != expected_counts:
        raise ValueError(f"Unexpected entry cohort counts: {entry_core['players'].tolist()}")
    if burn_in["post_burn_in_first_appearances"].tolist() != [4829, 4637, 4373, 3710]:
        raise ValueError("Burn-in sensitivity counts do not match the frozen definitions")

    source = Path(comparison_input_path)
    if not source.is_absolute():
        source = PROJECT_ROOT / source
    scores = pd.read_csv(source, low_memory=False)
    debut = _comparison_debut_probabilities(scores)
    full = diagnostics.loc[
        diagnostics["first_recorded_year"].eq(TEST_YEAR),
        ["match_id", "player_id", "p_focal_current_convention"],
    ]
    aligned = full.merge(debut, on=["match_id", "player_id"], validate="one_to_one")
    maximum = float(
        np.max(
            np.abs(
                aligned["p_focal_current_convention"] - aligned["comparison_probability"]
            )
        )
    )
    if len(aligned) != EXPECTED_DEBUT_APPEARANCES or maximum > 1e-12:
        raise ValueError("Formal 2025 entry probabilities do not match the unified input")

    root = Path(output_root)
    if not root.is_absolute():
        root = PROJECT_ROOT / root
    destination = ensure_directory(root.resolve() / "entry_diagnostics")
    figure_dir = ensure_directory(destination / "figures")
    paths = {
        "classification": destination / "player_entry_classification.csv",
        "diagnostics": destination / "prematch_entry_diagnostics.csv",
        "entry_core": destination / "entry_cohort_definitions_core.csv",
        "burnin_core": destination / "burnin_sensitivity_core.csv",
        "scale_core": destination / "prematch_scale_alignment_2025_core.csv",
        "orientation_core": destination / "orientation_sensitivity_2025_core.csv",
        "figure": figure_dir / "prematch_scale_alignment_2025_replot.png",
    }
    classification.to_csv(paths["classification"], index=False)
    diagnostics.to_csv(paths["diagnostics"], index=False)
    entry_core.to_csv(paths["entry_core"], index=False)
    burn_in[
        [
            "burn_in_years",
            "post_burn_in_start_year",
            "post_burn_in_first_appearances",
            "test_year_first_appearances_retained",
        ]
    ].to_csv(paths["burnin_core"], index=False)
    scale_core.to_csv(paths["scale_core"], index=False)
    orientation_core.to_csv(paths["orientation_core"], index=False)
    create_prematch_scale_alignment_replot(scale_core, paths["figure"])
    return paths
